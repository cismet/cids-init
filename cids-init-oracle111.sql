...
INSERT INTO cs_type ( name, class_id, complex_type, descr, editor, renderer) VALUES ( 'CLOB', NULL, '0', NULL, NULL, NULL);
...
INSERT INTO cs_attr ( class_id, type_id, name, field_name, foreign_key,pos,optional) VALUES ( (SELECT id FROM cs_class WHERE name = 'URL_BASE'), (SELECT id FROM cs_type WHERE name = 'CLOB'), 'PATH', 'PATH', '0',   3,'0');
INSERT INTO cs_attr ( class_id, type_id, name, field_name, foreign_key,pos,optional) VALUES ( (SELECT id FROM cs_class WHERE name = 'URL_BASE'), (SELECT id FROM cs_type WHERE name = 'CLOB'), 'SERVER', 'SERVER', '0',   2,'0');
...
INSERT INTO cs_attr ( class_id, type_id, name, field_name, foreign_key,pos,optional) VALUES ( (SELECT id FROM cs_class WHERE name = 'URL'), (SELECT id FROM cs_type WHERE name = 'CLOB'), 'OBJECT_NAME', 'OBJECT_NAME', '0', 2, '0');
...
BEGIN
  DELETE FROM cs_attr_object_derived_temp;
	...
	INSERT INTO cs_attr_object_derived_temp 
    SELECT CONNECT_BY_ROOT class_id xcid, CONNECT_BY_ROOT object_id xoid, attr_class_id acid, attr_object_id aoid
    FROM cs_attr_object
    START WITH class_id = classid
    CONNECT BY (PRIOR attr_class_id = class_id AND PRIOR attr_object_id = object_id AND level < 1000)
    
    UNION ALL
    
    SELECT DISTINCT class_id xcid, object_id xoid, class_id acid, object_id aoid FROM cs_attr_object WHERE class_id = classid
...
CREATE OR REPLACE PROCEDURE reindex_single(class_id integer)
  IS
    CURSOR c1 IS SELECT child cid
                  FROM cs_class_hierarchy
                  START WITH father = class_id
                  CONNECT BY PRIOR child = father

                  UNION ALL
                  
                  SELECT class_id cid FROM DUAL;
    CURSOR c2 IS SELECT c.cid FROM (             
                  SELECT child cid
                  FROM cs_class_hierarchy
                  START WITH father = class_id
                  CONNECT BY PRIOR child = father
                  ...
                  JOIN cs_class ON id = c.cid WHERE indexed = '1' AND rownum <= 100;
BEGIN
	FOR r IN c1 LOOP
		reindexpure_class(abs(r.cid));
	END LOOP;

	FOR r IN c2 LOOP
		reindexderivedobjects(abs(r.cid));
	END LOOP;
END;
/
